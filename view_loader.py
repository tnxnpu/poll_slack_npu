...
def get_create_poll_modal(trigger_id: str, channel_id: Optional[str] = None,
    ...
    if draft_state:
        ...
        # 2. Find the position where the new choices should be inserted.
        ...
        # 3. Create and populate new choice blocks from the draft data and insert them.
        ...
        for i, (block_id, block_data) in reversed(list(enumerate(draft_choices))):
            ...
            # Create the element and populate its initial_value at the same time
            element = {
        ...
        # 5. Populate the remaining blocks (question, settings, channel) with saved values.
        for block_id, block_data in draft_state.items():
            # Skip choice blocks as they are already handled
            ...
            for block in view_payload.get("blocks", []):
                if block.get("block_id") == block_id:
                    ...
                    if element_type == "plain_text_input":
                        ...
                        conversations = value_dict.get("selected_conversations")
                        if conversations:  # avoid []
                            element["initial_conversations"] = conversations
                    elif element_type == "checkboxes":
                        options = value_dict.get("selected_options")
                        if options:  # avoid []
                            element["initial_options"] = options
                    break
